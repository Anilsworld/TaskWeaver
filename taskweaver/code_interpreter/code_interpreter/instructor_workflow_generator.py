    ...
    def generate_workflow(
        ...
        try:
            ...
            error_msg = (
                ...
                f"4. Check that all node IDs in edges actually exist"
            )
...
4. **PARALLEL EXECUTION**
   ...
   **WHEN TO USE PARALLEL:**
   ...
   - ❌ NOT for sequential steps (use regular nodes with sequential edges)
   - ❌ NOT when one task needs another's output (use sequential edges)

5. **NODE DEPENDENCIES AND EDGES**
   - CRITICAL: Follow the Planner's step sequence from init_plan
   - If plan says "step B <sequential dependency on A>", create edge: A → B
   - Use 'edges' array: [{'from': 'node_a', 'to': 'node_b', 'type': 'sequential'}, ...]
   - NEVER create parallel edges unless the Planner explicitly allows it
   - Edges ensure correct execution order matching the Planner's logic
...
"""
    ...
    def _build_user_prompt(
        ...
        prompt_parts.append(
            ...
            "2. Create 'edges' array following the Planner's step sequence (if plan says step B depends on A, add edge A→B)\n"
            "3. Use sequential edges for dependencies, parallel edges ONLY when Planner explicitly allows\n"
    ...
    def _post_process_workflow(self, workflow: EnhancedWorkflowDefinition) -> EnhancedWorkflowDefinition:
        ...
        Post-processing only handles edge validation (if needed in future).
        Currently minimal to avoid over-fixing issues that should be caught in generation.
        """
        issues_fixed = []
        
        # Minimal post-processing - edges should be generated by LLM directly following Planner's sequence
